...
if (!empty($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) == 'xmlhttprequest'){
	...
	//if($nueva_consulta = $mysqli->prepare("SELECT nombre, skin,val FROM usuarios2020 WHERE usuario = ? AND pass = ? AND skin = 'electoral'")){
...
INNER JOIN  tipousuarios tu ON tu.idtipoUser = us.idtipoUser WHERE us.usuario = ? AND us.pass = ? AND lp.skin = 'electoral'")){
