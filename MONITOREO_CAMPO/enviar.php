...
function enviarEmail($email,$url)
{
	...
	$para  = $email;
	$título = 'ACTUALIZACIÓN DE CONTRASEÑA';
	// mensaje
		...
		<tr>
			<td style="background-color: #ecf0f1">
				<div style="color: #34495e; margin: 4% 10% 2%; text-align: justify;font-family: sans-serif">
					...
					Se ha actualizado correctamente tu contraseña para el acceso a '.$skin_global.'.</p>
...
function emailExiste($email,$skin)
{
	...
	$stmt = $mysqli->prepare("SELECT us.idUsuario, us.nombre, us.usuario, us.pass,us.val,us.skin, us.idtipoUser, us.idlink, lp.skin, lp.link from usuarios2020 us
	INNER JOIN linkspbi lp ON lp.idlink = us.idlink
	INNER JOIN  tipousuarios tu ON tu.idtipoUser = us.idtipoUser WHERE us.usuario = ? AND lp.skin = ? LIMIT 1");
...
function getValor2($campo, $valor,$skin)
{
	...
	$stmt = $mysqli->prepare("SELECT us.$campo from usuarios2020 us
	INNER JOIN linkspbi lp ON lp.idlink = us.idlink
	INNER JOIN  tipousuarios tu ON tu.idtipoUser = us.idtipoUser WHERE us.usuario = ? AND lp.skin = ? LIMIT 1");
		...
		<tr>
			<td style="background-color: #ecf0f1">
				<div style="color: #34495e; margin: 4% 10% 2%; text-align: justify;font-family: sans-serif">
					<h2 style="color: #309e04; margin: 0 0 7px">Hola!</h2>
					<p style="margin: 2px; font-size: 15px">
						Se ha solicitado la recuperación de su contraseña para '.$skin_global.'. <br/><br/>Haz clic en el siguiente botón:</p>
					<br>
					<div style="width: 100%; text-align: center">
